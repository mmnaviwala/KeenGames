Shader "Vertex Paint/Vertex Blend Advanced 4 Textures/Normal"
{
	properties {
	...
	subshader {
		...
		inline fixed4 Blend (fixed4 val, fixed4 val2, fixed2 uv_Noise, fixed r, fixed noise)
        {
			fixed term = pow( (r + ( (1-r) * noise) + (noise * r) * 1 ), 16 );
			term = clamp( term, 0.0, 1.0 );
			return lerp(val,val2,term);
        }  
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			// diffuse color
			fixed4 test = Blend(Splat1.rgba,Splat2.rgba,uv_Noise,splat_control.g,noise);
			test = Blend(test.rgba,Splat3.rgba,uv_Noise,splat_control.b,noise);
			test = Blend(test.rgba,Splat4.rgba,uv_Noise,splat_control.a,noise);
			albedo = test;
			//normals
			tempNormal = Blend(tex2D(_Splat1bump, uv1),tex2D(_Splat2bump, uv2),uv_Noise,splat_control.g,noise);
			tempNormal = Blend(tempNormal.rgba,tex2D (_Splat3bump, uv3),uv_Noise,splat_control.b,noise);
			tempNormal = Blend(tempNormal.rgba,tex2D (_Splat4bump, uv4),uv_Noise,splat_control.a,noise);
			...
			//o.Albedo = lerp(Splat1, Splat2, term) * _Color;
			o.Gloss = gloss;
