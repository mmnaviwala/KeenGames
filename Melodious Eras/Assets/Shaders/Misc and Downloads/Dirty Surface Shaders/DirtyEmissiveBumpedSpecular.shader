Shader "_DirtyEmissiveBumpedSpecular"
{
	Properties 
	{
...
float4 Multiply6=_SpecularColor * _SpecularIntensity.xxxx;
float4 Tex2D7=tex2D(_Dirt,(IN.uv_Dirt.xyxy).xy);
float4 Tex2D8=tex2D(_DirtIntensity,(IN.uv_DirtIntensity.xyxy).xy);
float4 Multiply7=Tex2D8 * _MakeDirty.xxxx;
float4 Lerp1=lerp(Multiply6,Tex2D7,Multiply7);
float4 Multiply1_1_NoInput = float4(0,0,0,0);
float4 Multiply1=Lerp1 * Multiply1_1_NoInput;
