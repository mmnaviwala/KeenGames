Shader "_DirtyEmissiveBumpedDiffuse"
{
	Properties 
	{
...
float4 Tex2D3=tex2D(_DirtIntensity,(IN.uv_DirtIntensity.xyxy).xy);
float4 Multiply5=Tex2D3 * _MakeDirty.xxxx;
float4 Lerp0=lerp(Tex2D0,Tex2D1,Multiply5);
float4 Multiply1=_DiffuseIntensity.xxxx * _DiffuseColor;
float4 Multiply0=Lerp0 * Multiply1;
float4 Tex2D2_1_NoInput = float4(0,0,0,0);
float4 Tex2D2=tex2D(_BumpMap,Tex2D2_1_NoInput.xy);
float4 UnpackNormal0=float4(UnpackNormal(Tex2D2).xyz, 1.0);
float4 Tex2D6=tex2D(_Emission,(IN.uv_Emission.xyxy).xy);
float4 Tex2D5=tex2D(_Dirt,(IN.uv_Dirt.xyxy).xy);
float4 Tex2D7=tex2D(_DirtIntensity,(IN.uv_DirtIntensity.xyxy).xy);
float4 Multiply6=Tex2D7 * _MakeDirty.xxxx;
float4 Lerp1=lerp(Tex2D6,Tex2D5,Multiply6);
float4 Multiply2=Lerp1 * _EmissionIntensity.xxxx;
