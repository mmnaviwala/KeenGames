...
float4 Tex2D2=tex2D(_DiffuseTexture,(IN.uv_DiffuseTexture.xyxy).xy);
float4 Tex2D3=tex2D(_Dirt,(IN.uv_Dirt.xyxy).xy);
float4 Tex2D4=tex2D(_DirtIntensity,(IN.uv_DirtIntensity.xyxy).xy);
float4 Multiply2=Tex2D4 * _MakeDirty.xxxx;
float4 Lerp1=lerp(Tex2D2,Tex2D3,Multiply2);
float4 Multiply1=_DiffuseColor * _DiffuseIntensity.xxxx;
float4 Multiply0=Lerp1 * Multiply1;
