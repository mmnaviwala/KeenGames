...
public partial class LightShafts : MonoBehaviour
{
	...
	void InitShadowmap()
	{
		bool _dynamic = (m_ShadowmapMode == LightShaftsShadowmapMode.Dynamic);
		if (_dynamic && m_ShadowmapMode != m_ShadowmapModeOld)
		{
		...
		InitRenderTexture(ref m_Shadowmap, m_ShadowmapRes, m_ShadowmapRes, 24, RenderTextureFormat.RFloat, _dynamic);
		m_Shadowmap.filterMode = FilterMode.Point;
		m_Shadowmap.wrapMode = TextureWrapMode.Clamp;

		if (m_Colored)
			InitRenderTexture(ref m_ColorFilter, m_ShadowmapRes, m_ShadowmapRes, 0, RenderTextureFormat.ARGB32, _dynamic);
